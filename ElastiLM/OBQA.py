...
dataset = datasets.load_dataset(path="allenai/openbookqa", name="main", split="validation")
...
# import random
# prompt_list = tokenizer.encode(TEMPLATE)
# num_to_keep = len(prompt_list) // 4
# kept_elements = random.sample(prompt_list, num_to_keep)
# TEMPLATE = tokenizer.decode(kept_elements)
    ...
    for i in tqdm(range(len(dataset))):
        ...
        if args.mode == "Ours": 
            ...
            for j in range(len(dataset[i]['choices']['text'])):
                NEW_QUERY = "Question: {question} Answer: {answer}".format(question=dataset[i]['question_stem'], answer=dataset[i]['choices']['text'][j])
        ...
        prompt = TEMPLATE.format(question=dataset[i]['question_stem'])
