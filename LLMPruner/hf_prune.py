...
def main(args):
    ...
    root_layers = list(set([i for i in range(len(model.model.layers))])-set(args.anchor_layers))
    ...
    if args.block_wise:
        kwargs = {
            ...
            "root_instances": [model.model.layers[i].self_attn.q_proj for i in root_layers] +
                              [model.model.layers[i].mlp.gate_proj for i in root_layers]
        }
        logger.log("Pruning Attention Layer = {}".format(root_layers))
        logger.log("Pruning MLP Layer = {}".format(root_layers))
...
if __name__ == "__main__":
    ...
    # added arguments
    parser.add_argument('--anchor_layers', type=int, nargs='+',
                    default=[],
                    help='an integer for the accumulator')
    parser.add_argument('--prune_pth', type=str, help='path')
    ...
    if "huggyllama/llama-7b" in args.base_model:
        scores.Passer.prune_strategy_save_path = "./ELASTICLLM/imp/Ours/llama/{pth}/".format(pth=args.prune_pth)
    elif "pankajmathur/orca_mini_3b" in args.base_model:
        scores.Passer.prune_strategy_save_path = "./ELASTICLLM/imp/Ours/orca_mini_3b/{pth}/".format(pth=args.prune_pth)
    elif "lmsys/vicuna-7b-v1.5" in args.base_model:
        scores.Passer.prune_strategy_save_path = "./ELASTICLLM/imp/Ours/vicuna/{pth}/".format(pth=args.prune_pth)
    
    print(scores.Passer.prune_strategy_save_path)
    main(args)
