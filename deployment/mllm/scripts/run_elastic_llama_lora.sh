...
mkdir ../models/orca_mini_3b-fp16-lora/
python3 gen_lora.py
...
if ! adb -H host.docker.internal shell [ -f "/data/local/tmp/mllm/models/orca_mini_3b-fp16-lora/" ]; then
    adb -H host.docker.internal push ../models/orca_mini_3b-fp16-lora/ /data/local/tmp/mllm/models/
else
    echo "orca_mini_3b-fp16 lora file already exists"
fi
...
adb -H host.docker.internal shell "cd /data/local/tmp/mllm/bin && ./demo_elastic_llama_lora -m ../models/orca_mini_3b-fp16.mllm -a ../models/orca_mini_3b-fp16-lora/ -v ../vocab/orca_vocab.mllm -t 1 -l 214"
