...
adb push ../bin-arm/demo_elastic_llama /data/local/tmp/mllm/bin/
# adb push ../models/llama-2-7b-chat-q4_k.mllm /data/local/tmp/mllm/models/
# adb push ../models/llama-2-7b-chat-fp16.mllm /data/local/tmp/mllm/models/

if ! adb shell [ -f "/data/local/tmp/mllm/models//llama-2-7b-chat-fp16.mllm" ]; then
    adb push ../models//llama-2-7b-chat-fp16.mllm "/data/local/tmp/mllm/models//llama-2-7b-chat-fp16.mllm"
else
    echo "/llama-2-7b-chat-fp16.mllm file already exists"
fi
...
adb shell "cd /data/local/tmp/mllm/bin && ./demo_elastic_llama"