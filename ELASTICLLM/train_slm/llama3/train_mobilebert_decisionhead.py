...
base_model = reorder_llama3(base_model)
...
scorehead = torch.load("ELASTICLLM/train_slm/llama3/slm_scorehead.pt")
...
for idx in tqdm(range(len(text_inputs))):
    ...
    if idx % 10 == 0:
        ...
        # 写入文件
        with open("ELASTICLLM/train_slm/llama3/in_out_ckpts/inputs_{}.json".format(str(idx)), 'w') as file:
            file.write(inputs_json_data)
        with open("ELASTICLLM/train_slm/llama3/in_out_ckpts/outputs_{}.json".format(str(idx)), 'w') as file:
            ...
            for model_ratio in model_ratios:
                for prompt_ratio in prompt_ratios:
                    if model_ratio*prompt_ratio <= prefill_slos[prefill_slo_idx] and model_ratio <= decode_slos[decode_slo_idx]:
                        # print(torch.cuda.memory_reserved()/1024/1024)
                        model_elastic = get_llama3(model=base_model, ratio=model_ratio)
...
torch.save(model, "ELASTICLLM/train_slm/llama3/slm_decisionhead_llama3.pt")