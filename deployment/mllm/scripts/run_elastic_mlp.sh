...
adb -H host.docker.internal push ../bin-arm/demo_elastic_mlp /data/local/tmp/mllm/bin/
adb -H host.docker.internal push ../models/elasticmlp-fp16.mllm /data/local/tmp/mllm/models/
# if push failed, exit
if [ $? -ne 0 ]; then
...
adb -H host.docker.internal shell "cd /data/local/tmp/mllm/bin && ./demo_elastic_mlp -m ../models/elasticmlp-fp16.mllm -t 4"