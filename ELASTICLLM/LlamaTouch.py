...
for i in range(len(dev_df)):
    ...
    SYS_PROMPT += "\nQuestion: {question} Answer: {answer}".format(question=question, answer=str(choice))
    if i==5:
        break
...
with torch.no_grad():
    for i in tqdm(range(len(val_df))):
        ...
        if args.mode == "Ours": 
            ...
            max_query_len = 0
            for j in apps:
                NEW_QUERY = "Question: {question} Answer: {answer}".format(question=val_df.iloc[i, 0], answer=str(j))
