...
dataset = datasets.load_dataset(path="gimmaru/piqa", split="validation")
...
with torch.no_grad():
    for i in tqdm(range(len(dataset))):
        ...
        if args.mode == "Ours": 
            ...
            max_query_len = 0
            for j in range(len([dataset[i]['sol1'], dataset[i]['sol2']])):
                NEW_QUERY = "Question: {question} Answer: {answer}".format(question=dataset[i]['goal'], answer=dataset[i]['sol{}'.format(j+1)])
        ...
        prompt = TEMPLATE.format(question=dataset[i]['goal'])
        # create answers
        batched_answers = []
        generated_texts = []
        for j in range(len([dataset[i]['sol1'], dataset[i]['sol2']])):
            answer = prompt + dataset[i]['sol{}'.format(j+1)]
            batched_answers.append(answer)
            generated_texts.append(dataset[i]['sol{}'.format(j+1)])
