        ...
        if self.global_pruning:
            ...
            if interactive:
                ...
                for group in self.prune_local():
                    prun_res = group.prune()
                    prune_strategy.append(prun_res)
                import json
                # 序列化数据
                json_data = json.dumps(prune_strategy)

                # 写入文件
                import scores, os
                directory = os.path.dirname(scores.Passer.prune_strategy_save_path+"rank_pruned.json")
                if not os.path.exists(directory):
                    os.makedirs(directory)
            
                with open(scores.Passer.prune_strategy_save_path+"rank_pruned.json", 'w') as file:
    ...
    def prune_local(self):
        ...
        for group in self.DG.get_all_groups(ignored_layers=self.ignored_layers, root_module_types=self.root_module_types, root_instances=self.root_instances):
            ...
            if self._check_sparsity(group):
                ...
                imps.append(imp.tolist())
                rankings.append(imp_argsort.tolist())
                if ch_groups > 1:
        ...
        import json
        # 序列化数据
        json_data = json.dumps(rankings)
        json_data2 = json.dumps(imps)
        # 写入文件
        import scores, os
        directory = os.path.dirname(scores.Passer.prune_strategy_save_path)
        if not os.path.exists(directory):
            os.makedirs(directory)
    
        with open(scores.Passer.prune_strategy_save_path+"rank_all.json", 'w') as file:
            file.write(json_data)
        with open(scores.Passer.prune_strategy_save_path+"imp.json", 'w') as file:
